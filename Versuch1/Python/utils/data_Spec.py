...
Spectrum data handling for viewer for spectrometer data
...
# create data array from SpecScale, SpecVals
def combineData(SpecScale, SpecVals):
    data = []
    for i in range(len(SpecScale)):
        data.append([SpecScale[i], SpecVals[i]])
    return data


# splits data array into SpecScale, SpecVals
def splitData(data):
    SpecScale = np.array(data)[ : , 0]
    SpecVals =  np.array(data)[ : , 1]
    return SpecScale, SpecVals
